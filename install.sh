...
clear
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To AndreSakti Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}ANDRE SAKTI ${NC}${YELLOW}(${NC} ${green}AndreSakti ${NC}${YELLOW})${NC}"
echo -e "       © Recode By AndreSakti ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
...
IZIN=$(curl https://raw.githubusercontent.com/andresakti7/kintil/main/ip | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
echo -e "             ${green}Telegram t.me/NetworkStore_ID "
...
# Save IP
IPVPS_SAVE=$(curl -sS ipv4.icanhazip.com)
echo "$IPVPS_SAVE" >/usr/bin/IPVPS_SAVE
# validity
rm -f /usr/bin/e
...
print_install "Membuat direktori xray"
    ...
    mkdir -p /var/lib/andresakti7 >/dev/null 2>&1
    # // Ram Information
...
echo "IP=" >> /var/lib/andresakti7/ipvps.conf
...
function password_default() {
    ...
    CHATID="6127364024"
    KEY="6448022205:AAFaPYRv83bUeupOWfvbCjVmY0S-ZAWaKjM"
    TIME="10"
...
#    rm -rf /etc/limit/vless
    ...
    rm -rf /etc/trojan/userexp
    rm -rf /etc/vless/userexp
    rm -rf /etc/vmess/userexp
    rm -rf /etc/ssh/userexp
#    rm -rf /usr/sbin/sakti
    mkdir -p /etc/xray
    mkdir -p /etc/vmess
    mkdir -p /etc/vless
    mkdir -p /etc/trojan
...
#    touch /etc/limit/vless
    ...
    touch /etc/trojan/userexp
    touch /etc/vless/userexp
    touch /etc/vmess/userexp
    touch /etc/ssh/userexp
#    touch /usr/sbin/sakti
...
wget "${REPO}ssh/udp-custom-linux-amd64" -O /etc/udp/udp-custom
chmod +x /etc/udp/udp-custom

echo downloading default config
wget "${REPO}ssh/config.json" -O /etc/udp/config.json
...
clear
# banner
echo "Banner /etc/andresakti7.txt" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/andresakti7.txt"@g' /etc/default/dropbear

# Ganti Banner
wget -O /etc/andresakti7.txt "${REPO}ssh/issue.net"
...
#    wget -O ~/menu.zip "${REPO}menu/menu.zip" >/dev/null 2>&1
    wget -O ~/menu.zip "https://saktitunneling.networkstoreid.asia:81/menu.zip" >/dev/null 2>&1
    mkdir /root/menu
    unzip -P SwipertDor2023!SwipertDor2023! menu.zip; -o/root/menu/ >/dev/null 2>&1
...
#    wget ${REPO}menu/botmin.zip
    wget https://saktitunneling.networkstoreid.asia:81/botmin.zip
    unzip -P Mekky999!Mekky999! botmin.zip
#    unzip botmin.zip
    mv adminbot /etc
    rm *.zip
...
echo -e "#  │▒│▒|▒│▒│     - Whatsapp   : 0859106848595           #" | lolcat
echo -e "# ┌┴─┴─┐-┘─┘     - Telegram   : t.me/NewtworkStore_ID   #" | lolcat
...
echo -e "    \033[0;32m│   - SlowDNS SSH             : ALL Port SSH  │"
...
echo -e "    \033[0;32m│   - SSH Payload Enhanced    : 80, 109       │"
echo -e "    \033[0;32m│   - SSH UDP-Custom          : 443, 1-65535  │"
